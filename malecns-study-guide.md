...
- 规模：**16.67 万神经元**（v1.0 坐实 166,700），社区过滤口径为 **25,582,938 条有向连接 / 124,177,617 个突触接触点**（两项均于 2026-09-20 用官方 v1.0 flat-connectome 坐实）。
...
| 项目 | 数值 | 说明 |
|---|---|---|
| 神经元 | **166,700**（v1.0 正式口径）/ 166,691（2025-10 预印本） | 2026-09-20 坐实：官方 v1.0 注释文件里 `superclass` 非空的 body **正好 166,700 个** |
| 有向连接（去重后） | **25,582,938** | 即"神经元 A → 神经元 B"的对数。**已坐实**（两端都是有 `superclass` 的神经元） |
| 突触接触点 | **124,177,617** | 接触点 ≠ 连接：一对神经元之间可以有多个突触。**已坐实** |
...
- ~~**166,691 与 166,700 的差异**来自不同过滤口径~~ → **已坐实（2026-09-20）**：预印本是 166,691，v1.0 正式口径是 **166,700**，等于官方注释文件里 `superclass` 非空的 body 数，实测正好相符。
- ~~**1.24 亿突触接触点**来自社区 flat-connectome 解析，可能不等于论文报告的突触总数~~ → **已坐实（2026-09-20）**：官方 v1.0 flat-connectome 按「两端都是有 `superclass` 的神经元」过滤，实测正好 **124,177,617** 个突触 / **25,582,938** 条边。
